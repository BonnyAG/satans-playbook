...
<main class="container mx-auto mt-4">
  ...
  <div class="grid grid-cols-4 grid-flow-row gap-y-8">
    {#each cards as card}
        <div class="bg-white border-8 border-surface-500 shadow-md shadow-surface-500 text-slate-900 p-4 w-72 rounded-2xl col-auto hover:scale-110 hover:shadow-primary-800 hover:shadow-2xl transition flex flex-col text-center h-[26rem] hover:cursor-pointer" on:click={() => goto('/cards/' + card.id)} on:keypress={() => goto('/cards/' + card.id)}>
